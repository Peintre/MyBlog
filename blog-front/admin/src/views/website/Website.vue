  ...
  <el-card class="main-card">
    <el-tabs v-model="activeName" @tab-click="handleClick">
      ...
      <el-tab-pane label="社交信息" name="notice">
        <el-form label-width="70px" :model="websiteConfigForm" size="default">
...
export default {
  ...
  methods: {
    getWebsiteConfig() {
      this.axios.get("/api/websiteConfig/getConfig").then(({ data }) => {
    ...
    updateWebsiteConfig() {
      this.axios
        .post("/api/websiteConfig/updateWebsiteConfig", this.websiteConfigForm)
        .then(({ data }) => {
          if (data.flag) {
            this.$notify.success({
              title: "成功",
